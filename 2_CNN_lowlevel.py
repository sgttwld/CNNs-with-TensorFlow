"""
A low-level implementation of a convolutional neural network 
for MNIST classification using tensorflow 
...
## model parameters
numIN,dimIN = 1, (28,28)				# number and size of input channels
numF, dimF = 4, (5,5) 					# number and size of conv filters (output: 4x24x24) 
dimC = (24,24)							# dimCout[i] = dimIN[i]-dimF[i] + 1 
dimPF = (2,2)							# size of pooling window (output: 4x12x12)
dimP = (int(dimC[0]/2),int(dimC[1]/2)) 	# dimPout[i] = dimCout[i]/2 
dimFLAT = numF*dimP[0]*dimP[1]			 # size of flattened input to the output layer
dimOUT = 10								# number of output nodes

## weights and bias
W = {
 	'c': tf.get_variable('conv_weights', (*dimF,numIN,numF), tf.float64, tf.glorot_uniform_initializer()),
	'y': tf.get_variable('dense_weights',(dimFLAT,dimOUT), tf.float64),
	}

b = {
	'c': tf.get_variable('weight_bias', (numF), tf.float64, tf.zeros_initializer()),
	'y': tf.get_variable('dense_bias', (dimOUT), tf.float64, tf.zeros_initializer()),
	}

## placeholders for data
X = tf.placeholder(tf.float64,[None,dimIN[0],dimIN[1],numIN])
Y = tf.placeholder(tf.int64,[None])
Y_1hot = tf.one_hot(Y,dimOUT,dtype=tf.float64)

## convolutional layer
C = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, W['c'], strides=(1,1,1,1), padding='VALID'),b['c']))

## pooling layer
P = tf.nn.avg_pool(C, (1,*dimPF,1),strides=(1,*dimPF,1), padding='VALID')

## flatten output of previous layers
FLAT = tf.layers.Flatten()(P)

## raw predictions
logits = tf.add(tf.matmul(FLAT,W['y']),b['y'])
